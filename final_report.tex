...
\end{equation}
where $r_{vel}$ rewards moving toward the target direction and $r_{face}$ rewards facing the target. This tests whether policies can generalize learned motions to directional control.
...
\textbf{Force Configurations.} We test three force scenarios:
\begin{itemize}
...
The 100N curriculum model exhibits more severe overcompensation, becoming unstable even without applied forces (Figure~\ref{fig:add_results}).
...
Table~\ref{tab:comparison} summarizes the key differences between AMP and ADD under force perturbations.
...
Our force curriculum reveals an important insight: the final curriculum force magnitude significantly affects generalization:
...
This suggests the need for \emph{adaptive curricula} that reduce force magnitudes in later training to encourage natural poses when forces are absent.
