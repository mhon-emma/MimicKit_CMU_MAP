...
AMP maintains more natural-looking poses and better pose tracking accuracy but exhibits lower force robustness:
\begin{itemize}
...
Figure~\ref{fig:amp_results} shows that AMP can walk with light constant forces but experiences drift and instability with strong perturbations, confirming the trade-off between pose tracking accuracy and force robustness.
...
Table~\ref{tab:pose_tracking} presents pose tracking error and episode length across force magnitudes. Key findings:
\begin{itemize}
\item ADD maintains lower pose tracking error under force but higher error at 0N (overcompensation)
\item Multi-directional training [10,10,30] reduces 0N pose error while maintaining robustness
\item AMP shows superior pose tracking at 0N but rapid degradation under force
\end{itemize}
...
% TODO: Fill in table with your experimental data
